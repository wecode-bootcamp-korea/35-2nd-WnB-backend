...
class DetailResrvationTest(TestCase):
    def setUp(self):
        ...
        Room.objects.create(
                    ...
                    price               = 10000,
                    description         = "산속에서 즐기는 휴식같은 하루",
                    latitude            = 123.0123456789,
                    longitude           = 127.0123456789,
                    maximum_occupancy   = 10,
                    bedroom             = 10,
                    bathroom            = 10,
                    bed                 = 10,
    ...
    def test_success_detail_reservation_get(self):
        client = Client()

        token       = jwt.encode({'id': User.objects.get(id=1).id}, settings.SECRET_KEY, settings.ALGORITHM)
        headers     = {'HTTP_Authorization' : token}
        response    = client.get('/reservations/100', **headers, content_type='application/json')
        ...
        body =  {
                "RESULT" : {
                    ...
                    'check_out'                 : "2022-08-10",
                    'images'                    : ["https://images.unsplash.com/photo-1610641818989-c2051b5e2cfd?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=1770&q=80",
                                                    "https://images.unsplash.com/photo-1582719508461-905c673771fd?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=1450&q=80",
